...
class MemoRepository {
    companion object{
        ...
        fun searchMemoDataByTitle(context: Context, searchWord:String) : MutableList<MemoModel>{
            val memoDataBase = MemoDataBase.getInstance(context)
            val memoVOList = memoDataBase?.memoDao()?.searchMemoData(searchWord)
            val memoModelList = mutableListOf<MemoModel>()

            memoVOList?.forEach {
                val memoModel = MemoModel(it.memoIdx, it.memoTitle, it.memoText, it.memoIsSecret, it.memoIsFavorite, it.memoPassword, it.memoCategoryIdx)
